...
class TestCompareRegimeMethods:
    def test_runs_end_to_end_and_produces_comparable_stats(self):
        # A literally flat series makes ADX mathematically undefined (zero
        # directional movement forever -> 0/0 in the +DI/-DI ratio, a
        # genuine NaN, not a bug) -- real market data always has some
        # tick-to-tick noise, so use a small-amplitude random walk instead,
        # just enough for every indicator (including ADX) to be well-defined.
        n = 400
        ...
        mtf_df = pd.DataFrame(
            {"close": 1.1000 + np.cumsum(rng.normal(0, 0.00003, 60))},
            index=pd.date_range("2025-12-28", periods=60, freq="4h", tz="UTC"),
        )

        folds, adx_summary, ml_summary = rco.compare_regime_methods(
            df, mtf_df, _symbol_info(),
            in_sample_bars=200, out_sample_bars=100,
            forward_bars=10, initial_balance=10_000.0, cfg=_cfg(),
        )

        assert len(folds) >= 1
        for fold in folds:
            assert isinstance(fold["label_threshold"], float)
